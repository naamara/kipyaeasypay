...
<div class="row">
  <div class="col-md-6">
    <h4 class="title"><?php echo lang('users title form_bills'); ?></h4>
  </div>
</div>
    ...
    <div class="panel panel-default">
      <div class="panel-body">
        <?php echo form_open(site_url("account/pay_bills/"), array("" => "")) ?>
		<?php $array = array(
				1 => "1 <-$-> UMEME <-$-> ".NULL, 2 => "2 <-$-> Water <-$-> ".NULL, 3 => "3 <-$-> GoTV <-$-> ".NULL, 4 => "4 <-$-> StarTimes <-$-> ".NULL, 5 => "5 <-$-> DSTV <-$-> ".NULL,
//				6 => "6 <-$-> UMEME <-$-> ".NULL,
		); ?>
        <div class="row">
          <div class="col-md-12">
            <div class="form-group">
              <label><?php echo lang('users withdrawal contact'); ?></label>
              <input type="number" class="form-control" name="receiver_contact" onkeyup="this.value = this.value.replace (/^\.|[^\d\.]/g, '')" placeholder="256788558138">
            </div>
          </div>
          <div class="col-md-12">
            <div class="form-group">
				<label><?php echo lang('billing item'); ?></label>
				<select name = "item_paid_for" class="form-control" >
				<?php foreach($array as $data){ $data = explode(" <-$-> ", $data); ?>
