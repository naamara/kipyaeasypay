...
//		var $username = '100929916075';
//		var $password = '1dxy-oGAf-OGON-sdnM-aizT-gRCf-ktHb-pQ9X';
...
//		require './YoPaymentsPHP/YoAPI.php';
		...
		$this->load->model('Airtimepurchase_model');
		$this->lang->load('currency');
		$this->load->library('YoPaymentsPHP/YoAPI');
		$this->load->library('commission');
		$this->load->library('email');
		
//		$yoAPI = new YoAPI("100929916075", "1dxy-oGAf-OGON-sdnM-aizT-gRCf-ktHb-pQ9X"); 
		// set constants
    ...
    */
	function start_new_code()
	{
		...
		if ($user['fraud_status']>0) {
			...
			$this->form_validation->set_rules('currency', lang('users transfer amount'), 'required|trim|in_list[debit_base,debit_extra1,debit_extra2,debit_extra3,debit_extra4,debit_extra5,debit_extra6]');
    ...
    */
	function history()
	{
        ...
        {
            $filters['time'] = $this->input->get('time', TRUE);
        }
	...
	function exchange_of_base_currency()
	{
		...
		elseif ($user['fraud_status']<2) {
			...
			else  
			{
				...
				} elseif ($currency == "debit_extra6") {
					$rates=$this->currencys->display->extra6_rate;
				}
    ...
    */
					...
					if ($this->form_validation->run() == FALSE)
						...
						else
						{
							...
							if ($method == "1") {
								...
								$method=lang('users withdrawal mtn');
								$com=$this->commission->display->payeer_fee;
							} elseif ($method == "11") {
	...
	/**
    * Mobile Withdrawal page
    */
	function mobile_withdrawal()
	{
        // setup page header data
        $this->set_title(sprintf(lang('users title mobwithdrawal'), $this->settings->site_name));
		// reload the new user data and store in session
        $user = $this->users_model->get_user($this->user['id']);
        ...
        // load views
        $data['content'] = $this->load->view('account/mobile_withdrawal', $content_data, TRUE);
		$this->load->view($this->template, $data);
	}
    ...
    */
	...
	function mobile_money_transfer()
	{
        // setup page header data
        $this->set_title(sprintf(lang('users title mobtransfer'), $this->settings->site_name));
		// reload the new user data and store in session
        $user = $this->users_model->get_user($this->user['id']);
        ...
        // load views
        $data['content'] = $this->load->view('account/mobile_money_transfer', $content_data, TRUE);
		$this->load->view($this->template, $data);
	}
	
	/**
    * Start mobile_withdrawal
    */
	function start_mobile_withdrawal()
	{
        ...
        $user = $this->users_model->get_user($this->user['id']);
			...
			if ($user['verifi_status']>=1 && $user['verifi_status']<3) {
				...
				elseif ($user['fraud_status']==0) {
					...
					$this->form_validation->set_rules('receiver_contact', lang('users withdrawal contact'), 'required|trim|numeric|max_length[12]|min_length[11]');
					$this->form_validation->set_rules('amount', lang('users transfer amount'), 'required|trim|numeric|greater_than[0]');
					$this->form_validation->set_rules('message', lang('users transfer amount'), 'required|trim');
					$this->form_validation->set_rules('method', lang('users transfer amount'), 'required|trim|in_list[1,2,3,4,5,6,7,8,9]');
						
//					if ($this->form_validation->run() == FALSE)
					if ($_POST['receiver_contact'] == NULL || $_POST['amount'] == NULL || $_POST['message'] == NULL || $_POST['method'] == NULL)
						{
							$this->session->set_flashdata('error', lang('users error form'));
							redirect(site_url("account/mobile_withdrawal"));
					}
					else
					{
						...
						$message = $this->input->post("message");
						$method = $this->input->post("method");
						$currency = 'debit_extra6';
							
						// Check method and fee
						if ($method == "10") {
							$method=lang('users withdrawal mtn');
							$com=$this->commission->display->mtn_fee;

							// Create a new YoAPI instance with Yo! Payments Username and Password
							$credentials = array(
									"username" => "100929916075",
									"password" => "1dxy-oGAf-OGON-sdnM-aizT-gRCf-ktHb-pQ9X",
							);
							$yoAPI = new YoAPI();//($username, $password); 
							$yoAPI->Settings($credentials);
									$percent = $com/"100";
									$fee = $amount*$percent;
									$yocharge = "390";
									$sum = $fee+$amount+$yocharge;
									$fee = ($yocharge + $fee);

									$total = $user['debit_extra6']-$sum;
									


								// Check wallet
								if ($user['debit_extra6']<$sum) {
								...
								elseif ($user['debit_extra6']>=$sum) {

								// Create a unique transaction reference that you will reference this payment with
								$transaction_reference = $amount.date("Y-m-d").rand(1,100);

								$yoAPI->set_external_reference($transaction_reference);

								$response = $yoAPI->ac_withdrawFunds($receiver_contact, $total, 'WITHDRAWAL OF FUNDS');

									if($response['Status']=='OK'){
										// Save this transaction for future reference
										// update wallet
//										$this->users_model->update_mobile_wallet_withdraw($user['username'],$total);
										$this->users_model->update_wallet_transfer($user['username'],
											array(
												$currency => $total,
											)
										...
										if($sms_template['enable']) {
											...
											array(
												...
												'from' => '+'.$this->settings->twilio_number,
												// the body of the text message you'd like to send
												'body' => "You have been sent a sum of shs ".$total
												)
											);
										...
										$this->session->set_flashdata('error', lang('users error service provider'));
										redirect(site_url("account/mobile_withdrawal"));
									}
								}else{
									$this->session->set_flashdata('error', lang('users error wallet'));
...
*/
    ...
    * Mobile Deposit
    */
	function mobile_deposit()
	{
        // setup page header data
        $this->set_title(sprintf(lang('users dashboard mobdeposit'), $this->settings->site_name));
		// reload the new user data and store in session
        $user = $this->users_model->get_user($this->user['id']);
        ...
        // load views
        $data['content'] = $this->load->view('account/mobile_deposit', $content_data, TRUE);
		$this->load->view($this->template, $data);
	}

	/**
    * Start mobile_deposit
    */
	function start_mobile_deposit()
	{
			
		// get the data
        $user = $this->users_model->get_user($this->user['id']);
			
		if ($user['verifi_status']>=0 && $user['verifi_status']<3) {
			
			// Check fraud 
			if ($user['fraud_status']>0) {
					
				$this->session->set_flashdata('error', lang('users error fraud'));
        		redirect(site_url("account/mobile_deposit"));
					
        	}
			
			elseif ($user['fraud_status']==0) {

				$this->form_validation->set_rules('sender_contact', lang('users input phone'), 'required|trim|numeric|max_length[12]|min_length[11]');
				$this->form_validation->set_rules('amount',  lang('users input amount'), 'required|trim|numeric|greater_than[0]');
				$this->form_validation->set_rules('message', lang('users input language'), 'required|trim');
				$this->form_validation->set_rules('method', lang('users transfer amount'), 'required|trim|in_list[1,2,3,4,5,6,7,8,9]');
					
//					if ($this->form_validation->run() == FALSE)
					if ($_POST['sender_contact'] == NULL || $_POST['amount'] == NULL || $_POST['amount'] <= 600 || $_POST['message'] == NULL || $_POST['method'] == NULL)
						{
							$this->session->set_flashdata('error', lang('users deposit error form'));
							redirect(site_url("account/mobile_deposit"));
						}
						else
						{
							...
							if ($method == "10") {
								...
								$fee = $amount*$percent;
								$yopercent = "3"/"100";
								$yocharge = ($yopercent * $amount);
								$dif = $amount - ($fee + $yocharge);
								$fee = ($yocharge + $fee);

								$total = $user['debit_extra6']+$dif;

								// Create a unique transaction reference that you will reference this payment with
								$transaction_reference = $sender_contact.date("Y-m-d").rand(1,100);

								$yoAPI->set_external_reference($transaction_reference);

								$response = $yoAPI->ac_deposit_funds($sender_contact, $amount, 'STOCK PURCHASE');
...
//								$this->users_model->update_mobile_wallet_withdraw($user['username'],$total);
								...
								$transactions = $this->transactions_model->add_transaction(array(
									...
									"currency"			=> $currency,
									"status" 			=> "2",
									"sender" 			=> $method,//"MTN Mobile Money ",//.$sender_contact."", //$user['username']
									"receiver" 			=> "system",
									"time"          	=> date('Y-m-d H:i:s'),
									"user_comment"  	=> $method.' '.$sender_contact.'',
									"admin_comment" 	=> "none"
									)
								);
								$method = 10;
								$deposit = $this->Deposit_model->add_stock(array(
									"mobile_sp" 		=> $method,
									"client" 			=> $user['id'],
									"mobile_number" 	=> $sender_contact,
									"type" 				=> "1",
									"sum"  				=> $dif,
									"fee"    			=> $fee,
									"amount" 			=> $amount,
								...
								if($sms_template['enable']) {
									...
									array(
										...
										'from' => '+'.$this->settings->twilio_number,
										// the body of the text message you'd like to send
										'body' => "You have deposited a sum of shs ".$amount." into your easypay account."
										)
									);

								}
								$this->session->set_flashdata('message', lang('users deposit success'));
								redirect(site_url("account/history"));
							...
							$currency = 'debit_extra6';
								$method=lang('users deposit airtel');
								$com=$this->commission->display->payeer_fee;
							...
							elseif ($user[$currency]>=$sum) {
								...
								$transactions = $this->transactions_model->add_transaction(array(
									...
									"currency"			=> $currency,
									"status" 			=> "2",
									"sender" 			=> $method,//$user['username'],
									"receiver" 			=> "system",
									"time"          	=> date('Y-m-d H:i:s'),
									"user_comment"  	=> $method.' '.$sender_contact.' '.$message.'',
									"admin_comment" 	=> "none"
									)
								);
    ...
    */
	...
	{
        // setup page header data
        $this->set_title(sprintf(lang('users title bills'), $this->settings->site_name));
		// reload the new user data and store in session
        $user = $this->users_model->get_user($this->user['id']);
        ...
        // load views
        $data['content'] = $this->load->view('account/bills', $content_data, TRUE);
		$this->load->view($this->template, $data);
	}
    ...
    */
	...
	{
        // setup page header data
        $this->set_title(sprintf(lang('users title airtime'), $this->settings->site_name));
		// reload the new user data and store in session
        $user = $this->users_model->get_user($this->user['id']);
        ...
        // load views
        $data['content'] = $this->load->view('account/airtime_purchase', $content_data, TRUE);
		$this->load->view($this->template, $data);
	}

	/**
    * Start mobile_airtime_purchase
    */
	function start_airtime_purchase()
	{
		...
		if ($user['verifi_status']>=1 && $user['verifi_status']<3) {
				...
				if ($user['fraud_status']>0) {
					...
					$this->session->set_flashdata('error', lang('users error fraud'));
					redirect(site_url("account/airtime_purchase"));
						
				}
			
				elseif ($user['fraud_status']==0) {
						
					$this->form_validation->set_rules('receiver_number', lang('users withdrawal contact'), 'required|trim|numeric|max_length[12]|min_length[11]');
					$this->form_validation->set_rules('amount', lang('users transfer amount'), 'required|trim|numeric|greater_than[0]');
					$this->form_validation->set_rules('method', lang('users transfer amount'), 'required|trim|in_list[1,2,3,4,5,6,7,8,9]');
						
	//					if ($this->form_validation->run() == FALSE)
						if ($_POST['receiver_number'] == NULL || $_POST['amount'] == NULL || $_POST['method'] == NULL)
							{
								$this->session->set_flashdata('error', lang('users error form'));
								redirect(site_url("account/airtime_purchase"));
							}
							else
							{
								...
								$receiver_contact = $this->input->post("receiver_number");
								$amount = $this->input->post("amount");
								...
								if ($method == "10") {
									...
									$com=$this->commission->display->mtn_fee;
										...
										// Create a unique transaction reference that you will reference this payment with
										$transaction_reference = $mobile_sp.date("Y-m-d").rand(1,100);
										$yoAPI->set_external_reference($transaction_reference);

										$response = $yoAPI->ac_send_airtime_mobile($receiver_contact, $amount, 'AIRTIME PURCHASE');

										if($response['Status']=='OK'){
										//	echo '<div role="alert" class="alert alert-success">Waiting for user to confirm mobile money transfer.You can check using this Transaction Reference = '.$response['TransactionReference'].'. Thank you for using Easypay</div>';
											// Save this transaction for future reference
...
//											$this->users_model->update_mobile_wallet_withdraw($user['username'],$total);
											...
											$airtime_topup = $this->Airtimepurchase_model->add_airtime_topup(array(
												"mobile_sp" 		=> $method,
												"client" 			=> $user['id'],
											...
											if($sms_template['enable']) {
												...
												array(
													...
													'from' => '+'.$this->settings->twilio_number,
													// the body of the text message you'd like to send
													'body' => "You have bought airtime of shs ".$amount
													)
												);
									...
									$this->session->set_flashdata('message', lang('users error service provider'));
									redirect(site_url("account/airtime_purchase"));
