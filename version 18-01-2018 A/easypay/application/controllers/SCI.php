			...
			if ($httpCode == 200) {
				...
				if ($_GET['confirmations'] >= 6) {
					...
					$post_data = array (  
						...
						"status" 				=> "Confirmed",
						"date" 					=> $date,
						"id_transfer" 			=> $transaction_hash,
						"id_method_transaction" => $transaction_hash,
						"ballance" 				=> $total,
						"custom" 				=> "none",
						"hash" 					=> $hash2
					); 
    ...
    */	
	public function sci_advcash()
	{
		...
		if($sha256==$_POST['ac_hash']){
			...
			$fee_percent = $amount*$percent;
		  	$fee = $fee_percent+$this->commission->display->sci_adv_fix;
			$fee = $amount*$percent;
			$date = date('Y-m-d H:i:s');
		...
		if (!in_array($_SERVER['REMOTE_ADDR'], array('185.71.65.92', '185.71.65.189', '149.202.17.210'))) return;
			...
			if (isset($_POST['m_operation_id']) && isset($_POST['m_sign']))
			{
				...
				if ($_POST['m_sign'] == $sign_hash && $_POST['m_status'] == 'success')
				{
					...
					$merchant_name = $detail_merchant['name'];
					$merchant_ipn = $detail_merchant['status_link'];
					$id_transfer = $_POST['m_operation_id'];
