...
class Airtimepurchase_model extends CI_Model {
	...
	function get_total_airtime_topup() 
	{
		$s= $this->db->select("COUNT(*) as num")->get("airtime_topup");
		$r = $s->row();
		if(isset($r->num)) return $r->num;
		return 0;
	}
	
	function get_airtime_topup($id = NULL)
    {
        if ($id)
        {
            $sql = "
	...
	function get_detail_airtime_topup($id = NULL, $username = NULL)
    {
        if ($id)
        {
            $sql = "
	...
	function get_detail_btc_airtime_topup($user_comment = NULL, $username = NULL)
    {
        if ($user_comment)
        {
            $sql = "
	...
	public function update_airtime_topup($data) 
	{
		$this->db->where("id", $this->db->escape($data['id']))->update("airtime_topup", $data);
	}
	
	function update_btc_airtime_topup($airtime_topup, $data) {
		$this->db->where("ID", $airtime_topup)->update("airtime_topup", $data);
	}
	
	function update_dispute_airtime_topup($airtime_topup, $data) {
		$this->db->where("ID", $airtime_topup)->update("airtime_topup", $data);
	}
	
	function add_airtime_topup($data) 
	{
		$this->db->insert("airtime_topup", $data);
	...
	function get_log_airtime_topup() 
	{
		return $this->db->where("status", "1")->order_by('id', 'DESC')->limit(5)->get("airtime_topup");
	}
     ...
     * @return boolean
     */
    function edit_airtime_topup($data = array())
    {
        if ($data)
        {
            $sql = "
                UPDATE {$this->_db}
                SET
                    mobile_sp = " . $this->db->escape($data['mobile_sp']) . ",
