...
class Deposit_model extends CI_Model {
	...
	{
		$s= $this->db->select("COUNT(*) as num")->get("stock");
		$r = $s->row();
		if(isset($r->num)) return $r->num;
		return 0;
	}
	
	function get_stock($id = NULL)
    {
        if ($id)
        {
            $sql = "
	...
	function get_detail_stock($id = NULL, $username = NULL)
    {
        if ($id)
        {
            $sql = "
	...
	function get_detail_btc_stock($user_comment = NULL, $username = NULL)
    {
        if ($user_comment)
        {
            $sql = "
	...
	{
		$this->db->where("id", $this->db->escape($data['id']))->update("stock", $data);
	}
	
	function update_btc_stock($stock, $data) {
		$this->db->where("ID", $stock)->update("stock", $data);
	}
	
	function update_dispute_stock($stock, $data) {
		$this->db->where("ID", $stock)->update("stock", $data);
	}
	...
	function get_log_stock() 
	{
		return $this->db->where("status", "1")->order_by('id', 'DESC')->limit(5)->get("stock");
	}
     ...
     * @return boolean
     */
    function edit_stock($data = array())
    {
        if ($data)
        {
            $sql = "
