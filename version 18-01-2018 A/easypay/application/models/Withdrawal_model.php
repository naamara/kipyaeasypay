...
class withdraws_model extends CI_Model {
	...
	function get_total_withdraws() 
	{
		$s= $this->db->select("COUNT(*) as num")->get("withdraws");
		$r = $s->row();
		if(isset($r->num)) return $r->num;
		return 0;
	}
	
	function get_withdraws($id = NULL)
    {
        if ($id)
        {
            $sql = "
	...
	function get_detail_withdraws($id = NULL, $username = NULL)
    {
        if ($id)
        {
            $sql = "
	...
	function get_detail_btc_withdraws($user_comment = NULL, $username = NULL)
    {
        if ($user_comment)
        {
            $sql = "
	...
	public function update_withdraws($data) 
	{
		$this->db->where("id", $this->db->escape($data['id']))->update("withdraws", $data);
	}
	
	function update_btc_withdraws($withdraws, $data) {
		$this->db->where("ID", $withdraws)->update("withdraws", $data);
	}
	
	function update_dispute_withdraws($withdraws, $data) {
		$this->db->where("ID", $withdraws)->update("withdraws", $data);
	}
	...
	function get_log_withdraws() 
	{
		return $this->db->where("status", "1")->order_by('id', 'DESC')->limit(5)->get("withdraws");
	}
     ...
     * @return boolean
     */
    function edit_withdraws($data = array())
    {
        if ($data)
        {
            $sql = "
