...
class Users_model extends CI_Model {
    ...
    function edit_user($data = array())
    {
        if ($data)
        {
            ...
            $sql .= "
                    ...
                    debit_extra6 = " . $this->db->escape($data['debit_extra6']) . ",
	...
	function update_mobile_wallet_withdraw($username, $data) {
//		$this->db->where("username", $username)->update("users","debit_extra6", $data);
		$sql = " UPDATE users SET debit_extra6 = " . $this->db->escape($data) . " WHERE username = " . $this->db->escape($username) . " ";
		$this->db->query($sql);
		if ($this->db->affected_rows())
		{
			return TRUE;
